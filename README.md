...
9. How do the Factory and Builder patterns work together in the MaterialDirector? What are the benefits of this combination?

The Factory and Builder patterns complement each other in the MaterialDirector to simplify and standardize the process of object creation within the bookstore system. The Factory pattern is responsible for deciding which subclass of Material to instantiate such as EBook, PrintedBook, or AudioBook based on input parameters or user selections. For example, the MaterialFactory or AdvancedMaterialFactory determines the correct material type dynamically, encapsulating the instantiation logic and promoting loose coupling between client code and concrete classes.

Once the appropriate type is determined, the Builder pattern takes over to handle step by step object construction, especially for complex or composite materials. The MaterialDirector coordinates the builder classes such as EBookBuilder, PrintedBookBuilder, or MaterialBundleBuilder to set specific attributes in a controlled order, ensuring that required fields are initialized properly and optional ones are added as needed. This approach is particularly useful for constructing composite structures like material bundles that require nested object creation.

By combining these two patterns, the system achieves both flexibility and control. The Factory handles the abstraction of type selection, while the Builder ensures that each object is assembled correctly and consistently. This separation of concerns reduces duplication, prevents invalid object states, and makes the system easier to extend. If a new material type or build process is introduced, developers can add a new factory case or builder subclass without modifying existing code. This collaboration between Factory and Builder ultimately enhances maintainability, extensibility, and readability of the object creation process in the domain layer.

10. Explain the Composite pattern implementation for material bundles. How does it enable recursive composition?

The Composite pattern in the Lab 4 bookstore system is implemented to allow both individual materials (like a single EBook or PrintedBook) and collections of materials (like bundles or packages) to be treated uniformly under a shared abstraction. The key classes involved are MaterialComponent, MaterialLeaf, and MaterialBundle. MaterialComponent serves as the abstract base class or interface that defines common operations such as getPrice() or getDescription(). The leaf nodes  represent individual items that implement these operations directly, while the composite nodes—like MaterialBundle contain a collection of MaterialComponent objects and implement the same interface by delegating or aggregating behavior.

For example, a MaterialBundle can hold a list of other materials, including both individual items and other bundles. When the getPrice() method is called on a bundle, it recursively sums up the prices of all contained components, whether they are single materials or nested bundles. This recursive structure enables hierarchical composition, meaning a bundle can contain other bundles to any depth, forming a tree like structure of materials.

This design provides two major benefits: uniformity and extensibility. Uniformity allows client code to treat individual materials and bundles the same way—operations like pricing, discounting, or display don’t need to differentiate between single items or collections. Extensibility allows developers to add new material types or bundle variations without modifying existing logic. The recursive composition achieved through the Composite pattern makes the system flexible and scalable, enabling the creation of complex product packages  from simpler components while maintaining clean and reusable code.
